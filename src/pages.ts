...
export function bankPickerPage(): Response {
  const body = `<h1>Choose a bank</h1><p class="lede">Choose Personal for privately owned accounts or Business for company accounts. Then search for your bank.</p><fieldset><legend>Account type</legend><div class="choices"><label class="choice"><input type="radio" name="psu" value="personal" checked><span><strong>Personal</strong><small>Privately owned accounts.</small></span></label><label class="choice"><input type="radio" name="psu" value="business"><span><strong>Business</strong><small>Company-owned accounts.</small></span></label></div><p class="hint">Connect the types in separate flows if you need both.</p></fieldset><div class="row"><div><label for="country">Country</label><select id="country"><option value="SE">Sweden (SE)</option></select></div><div><label for="q">Search bank</label><input id="q" placeholder="SEB, Swedbank, Nordea…" autocomplete="off"></div></div><p class="hint" id="meta" role="status" aria-live="polite">Loading banks…</p><div class="banks" id="list"></div><script>(function(){const countryEl=document.getElementById("country"),qEl=document.getElementById("q"),listEl=document.getElementById("list"),metaEl=document.getElementById("meta");let banks=[];function psu(){return(document.querySelector("input[name=psu]:checked")||{}).value||"personal"}function render(){const country=countryEl.value,q=(qEl.value||"").toLowerCase().trim(),type=psu(),rows=banks.filter(b=>(!country||b.country===country)&&(!q||b.name.toLowerCase().includes(q))&&(!b.psu_types||b.psu_types.includes(type))).sort((a,b)=>a.name.localeCompare(b.name));metaEl.textContent=rows.length+" bank"+(rows.length===1?"":"s");listEl.replaceChildren();rows.slice(0,200).forEach(b=>{const a=document.createElement("a"),params=new URLSearchParams({bank:b.name,country:b.country,psu:type});a.href="/auth/start?"+params.toString();a.append(document.createTextNode(b.name));const cc=document.createElement("span");cc.className="cc";cc.textContent=b.country;a.append(cc);listEl.append(a)})}function countries(codes){const preferred=["SE","NO","DK","FI","DE","GB","NL","FR","ES","IT"],sorted=Array.from(new Set(codes)).sort((a,b)=>{const ia=preferred.indexOf(a),ib=preferred.indexOf(b);if(ia!==-1||ib!==-1)return(ia===-1?99:ia)-(ib===-1?99:ib);return a.localeCompare(b)});countryEl.replaceChildren();const all=document.createElement("option");all.value="";all.textContent="All countries";countryEl.append(all);sorted.forEach(code=>{const option=document.createElement("option");option.value=code;option.textContent=code==="SE"?"Sweden (SE)":code;option.selected=code==="SE";countryEl.append(option)});if(!sorted.includes("SE"))countryEl.value=""}async function load(){try{const response=await fetch("/auth/banks");if(!response.ok)throw new Error();const data=await response.json();banks=data.banks||[];countries(data.countries?.length?data.countries:banks.map(b=>b.country));render()}catch{metaEl.setAttribute("role","alert");metaEl.textContent="Could not load banks. Check the Enable Banking credentials."}}countryEl.addEventListener("change",render);qEl.addEventListener("input",render);document.querySelectorAll("input[name=psu]").forEach(el=>el.addEventListener("change",render));load()})();</script>`;
  return pageResponse({ title: "Choose a bank", body });
}
