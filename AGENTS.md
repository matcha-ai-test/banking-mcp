...
10. Configure the requesting MCP client only if the user asked you to do so. Use `MCP_URL` and `CONNECTION_PASSWORD` from `.mcp-credentials` (the installer prints only the file path, never the values); do not commit either. For claude.ai the most reliable client configuration is No sign-in plus a request header named `x-api-key` holding `CONNECTION_PASSWORD`, which avoids the OAuth browser redirect entirely.
