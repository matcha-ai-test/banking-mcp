...
- Add or update a test when you change behavior, especially anything in the auth flow, the MCP tool output, or secret handling.
...
- Never put a token, password, IBAN, account identifier, or personal detail into code, tests, logs, commit messages, or CI output. The operator token (`START_TOKEN`) must stay out of request URLs; it travels only in the link fragment.
- MCP tool responses must not carry secrets or upstream session identifiers. If you touch `src/mcp.ts`, `src/mcp-output.ts`, or `src/db.ts`, check that the response still exposes only the allowed fields.
